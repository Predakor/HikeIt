...
{
    public static async Task TryMigrationAndSeeding(IServiceProvider services, IConfiguration configuration)
    {
        ...
        {
            Console.WriteLine($"Configuration: {configuration.GetConnectionString("TripDbCS")}");
            Console.Write($"Migration error: {ex}");
