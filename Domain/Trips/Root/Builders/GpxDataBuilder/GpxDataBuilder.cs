...
public sealed class MutableGpxPoint(double lat, double lon, double ele, DateTime? time = null) : IGeoPoint
{
...
{
    private List<MutableGpxPoint> _gpxPoints = [.. points.Select(p => p.ToMutable())];
    ...
    {
        _gpxPoints = _gpxPoints.DownSample(amount);
        return this;
    }
